...
int h[MAXN], Sum[MAXN], cnt, U[MAXN], Q[MAXN], trans[15][MAXN];
double f[15][MAXN];
double K(data &a, data &b) {
  return (b.y - a.y) / (b.x - a.x);
}
int main() {
  int n = read(), k = read(), p = read();
  for (int i = 1; i <= n; ++i) h[i] = read();
  for (int i = 2; i <= n; ++i) if (h[i] > h[1]) U[++cnt] = h[i];
  U[++cnt] = h[1];
  sort(U + 1, U + cnt + 1);
  for (int i = 1; i <= cnt; ++i) U[i] += U[i - 1];
  for (int i = 1; i <= cnt; ++i) f[i][0] = U[1];
  int lim = min(14, k);
  for (int i = 1; i <= lim; ++i) {
    f[i][1] = U[1];
    int h = 0, t = 0;
    Q[t] = 1;
    d[1].x = 0, d[1].y = U[1] - f[i - 1][1];
    for (int j = 2; j <= cnt; ++j) {
      now.x = j, now.y = U[j];
      while (h < t && K(d[Q[h]], now) < K(d[Q[h + 1]], k)) h++;
      d[j].x = j - 1, d[j].y = U[j] - f[i - 1][j];
      f[i][j] = (U[j] - (U[Q[h]] - f[i - 1][Q[h]])) / (j - Q[h] + 1);
      trans[i][j] = Q[h];
      while (h < t && K(d[Q[t - 1]], d[Q[t]]) > K(d[Q[t]], d[j])) t--;
      Q[++t] = j;
    }
  }
  int u = n - (k - lim);
