...
void Read(int X1, int Y1, int X2, int Y2, int i) {
...
int main() {
  scanf ("%d", &n);
  for (int i = 1; i <= n; i++) {
    scanf ("%d%d%d%d", &cc[i].X1, &cc[i].Y1, &cc[i].X2, &cc[i].Y2);
    Read(cc[i].X1, cc[i].Y1, cc[i].X2, cc[i].Y2, i);
  }
  bool ans = 0;
  ans |= RUN();
  clear();
  for (int i = 1; i <= n; i++) {
    // scanf ("%d%d%d%d", &cc[i].X1, &cc[i].Y1, &cc[i].X2, &cc[i].Y2);
    Read(cc[i].Y2, -cc[i].X1, cc[i].Y1, -cc[i].X2, i);
  }
  ans |= RUN();
  printf ("%d\n", ans);
}