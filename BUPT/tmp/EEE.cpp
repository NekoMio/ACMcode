...
data pow_mod(data a, long long b) {
  ...
  while (b) {
    if (b & 1)
	{
		a1=1ll*ans.A*a.A%MOD;
		b1=1ll*a.C*ans.A%MOD;
		c1=1ll*ans.C*a.A%MOD;
		d1=1ll*ans.C*a.C%MOD;
		ans.A=(1ll*a1+1ll*d1*D)%MOD;
		ans.C=(1ll*b1+1ll*c1)%MOD;
	}// ans = ans * a;
    b >>= 1;
    a1=1ll*a.A*a.A%MOD;
	b1=1ll*a.C*a.A%MOD;
	c1=1ll*a.C*a.A%MOD;
	d1=1ll*a.C*a.C%MOD;
	a.A=(1ll*a1+1ll*d1*D)%MOD;
	a.C=(1ll*b1+1ll*c1)%MOD;
    //a = a * a;
  }
  return ans;
}
